...
- IK 是单周期计算量的主体（约 14 us），PID 与 APF 修正都在亚微秒量级；相对 10 ms 控制周期，
  单周期计算量占比约 0.2%。
- 计时口径是主机 x86-64、-O2 的算法耗时；QEMU 的 `mps2-an386` 不是周期精确模型，因此不计算
   MCU 等效主频下的耗时。闭环中 70 ms/控制周期的挂钟时间由仿真步进与 UART 往返决定，
  不代表固件计算量。

### 6.2 栈与堆占用

FreeRTOS 堆 64 KiB（heap_4）；任务栈深度按 FreeRTOS 建栈时写入的 0xA5 填充统计，由脚本内置的
GDB 探针在长跑过程中读取。
...
- 堆：空闲稳定在 40.0 KiB（62%），历史最小剩余 37.9 KiB，6 个循环（6054 个控制
